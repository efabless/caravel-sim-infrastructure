        ...
        if reg1 != cpu.read_debug_reg1():
            ...
            if reg1 in pass_list:  # pass phase
                ...
                if reg1 == 0x1B:
                    ...
                    cocotb.log.info(
                        f"[TEST] Pass timer0 isn't changing after it reachs 0"
                    )
            elif reg1 in fail_list:  # pass phase
                if reg1 == 0x1E:
                    cocotb.log.info(
                        f"[TEST] Failed timer0 value increasing not decresing in oneshot mode"
                    )
                elif reg1 == 0x2E:
                    cocotb.log.error(
                        f"[TEST] Failed timer0 is changing before it reachs 0 in oneshot mode"
                    )
            else:
    ...
    while True:
        ...
        if reg1 != cpu.read_debug_reg1():
            ...
            if reg1 in pass_list:  # pass phase
                ...
                if reg1 == 0xEE:
                    cocotb.log.info(
                        f"[TEST] Failed timer0 value hasn't rollovered in periodic mode"
                    )
            else:
                cocotb.log.error(f"[TEST] debug register 1 has illegal value")
