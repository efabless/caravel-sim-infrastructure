...
async def IRQ_uart_rx(dut):
    ...
    cocotb.log.info(f"[TEST] Start IRQ_uart_rx test")
    pass_list = (0x1B, 0x2B)
    fail_list = (0x1E, 0x2E)
    phases_fails = 2
    phases_passes = 0
    ...
    while True:
        ...
        if reg1 != cpu.read_debug_reg1():
            ...
            if reg1 in pass_list:  # pass phase
                ...
                if reg1 == 0x1B:
                    cocotb.log.info(
                        f"[TEST] Pass interrupt is detected when uart is sending data"
                    )
                elif reg1 == 0x2B:
                    cocotb.log.info(
                        f"[TEST] Pass interrupt isn't detected when uart isnt sending data"
                    )
            elif reg1 in fail_list:  # pass phase
                if reg1 == 0x1E:
                    cocotb.log.info(
                        f"[TEST] Failed interrupt isn't detected uart is sending data"
                    )
                elif reg1 == 0x2E:
                    cocotb.log.error(
                        f"[TEST] Failed interrupt is detected uart isnt sending data"
                    )
            else:
