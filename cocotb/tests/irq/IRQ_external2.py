...
async def IRQ_external2(dut):
    caravelEnv,clock = await test_configure(dut,timeout_cycles=11155225)
    cpu = RiskV(dut)
    cpu.cpu_force_reset()
    ...
    while True: 
        if reg2 != cpu.read_debug_reg2():
            ...
            if reg2 == 0xAA:  # assert mprj 12 
                ...
                cocotb.log.info(f"irq 2 = {dut.uut.chip_core.housekeeping.irq_2_inputsrc.value}")
        ...
        if reg1 != cpu.read_debug_reg1():
            ...
            if reg1 in pass_list:  # pass phase
                ...
                phases_fails  -=1
                if reg1 == 0x1B:
                    cocotb.log.info(f"[TEST] Pass interrupt is detected when mprj 12 asserted")  
                elif reg1 == 0x2B:
                    cocotb.log.info(f"[TEST] Pass interrupt isn't detected when mprj 12 deasserted")   
            elif reg1 in fail_list:  # pass phase
                if reg1 == 0x1E:
                    cocotb.log.error(f"[TEST] Failed interrupt isn't detected when mprj 12 asserted")  
                elif reg1 == 0x2E:
                    cocotb.log.error(f"[TEST] Failed interrupt is detected when mprj 12 deasserted") 
            else: 
