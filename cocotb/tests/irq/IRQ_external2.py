    ...
    while True:
        ...
        if reg1 != cpu.read_debug_reg1():
            ...
            if reg1 in pass_list:  # pass phase
                ...
                if reg1 == 0x1B:
                    cocotb.log.info(
                        f"[TEST] Pass interrupt is detected when mprj 12 asserted"
                    )
                elif reg1 == 0x2B:
                    cocotb.log.info(
                        f"[TEST] Pass interrupt isn't detected when mprj 12 deasserted"
                    )
            elif reg1 in fail_list:  # pass phase
                if reg1 == 0x1E:
                    cocotb.log.error(
                        f"[TEST] Failed interrupt isn't detected when mprj 12 asserted"
                    )
                elif reg1 == 0x2E:
                    cocotb.log.error(
                        f"[TEST] Failed interrupt is detected when mprj 12 deasserted"
                    )
            else:
