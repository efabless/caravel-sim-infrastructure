...
async def gpio_all_o_caravan(dut):
    ...
    for j in range(5):
        ...
        expected_data = (
            bin(i << 32)[2:].zfill(38)[:13]
            + "zzzzzzzzzzz"
            + bin(i << 32)[2:].zfill(38)[24:]
        )
...
async def gpio_all_i_caravan(dut):
    ...
    if cpu.read_debug_reg2() == data_expected:
        ...
        cocotb.log.error(
            f"[TEST] Error: reg_mprj_datal has recieved wrong data {bin(cpu.read_debug_reg2())} instead of {bin(data_expected)}"
        )
    data_in = 0xAAAAAAAA
    cocotb.log.info(f"[TEST] drive {hex(data_in)} to gpio[31:0]")
    caravelEnv.drive_gpio_in((31, 0), data_in)
    ...
    if cpu.read_debug_reg2() == data_in:
        ...
        cocotb.log.error(
            f"[TEST] Error: reg_mprj_datah has recieved wrong data {cpu.read_debug_reg2()} instead of {data_in}"
        )
    data_in = 0x0
    cocotb.log.info(f"[TEST] drive {hex(data_in)} to gpio[37:32]")
    caravelEnv.drive_gpio_in((37, 32), data_in)
    ...
    if (
        caravelEnv.monitor_gpio((37, 0)).binstr
...
async def gpio_all_bidir(dut):
    ...
    if cpu.read_debug_reg2() == data_in:
        ...
        cocotb.log.error(
            f"[TEST] Error: reg_mprj_datal has recieved wrong data {cpu.read_debug_reg2()} instead of {data_in}"
        )
    data_in = 0xFFA88C5A
    cocotb.log.info(f"[TEST] drive {hex(data_in)} to gpio[31:0]")
    caravelEnv.drive_gpio_in((31, 0), data_in)
