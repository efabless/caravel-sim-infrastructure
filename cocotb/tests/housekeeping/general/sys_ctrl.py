...
async def hk_disable(dut):
    ...
    if pll_enable == 1 - old_pll_enable:
        ...
        cocotb.log.info(
            f"[TEST] pass: SPI isn't working when SPI housekeeping is disabled"
        )
    ...
    if pll_enable == 1 - old_pll_enable:
        cocotb.log.info(
            f"[TEST] Pass: Housekeeping SPI has been enabled correctly through firmware"
        )
    else:
        cocotb.log.error(
            f"[TEST] Error: Housekeeping SPI failed to be enabled through firmware"
        )
