...
async def gpio_all_o(dut):
    caravelEnv,clock = await test_configure(dut,timeout_cycles=1111538624)
    await gpio_all_o_seq(dut,caravelEnv,clock)

@cocotb.test()
@repot_test
async def gpio_all_i(dut):
    caravelEnv,clock = await test_configure(dut,timeout_cycles=111156837)
    await gpio_all_i_seq(dut,caravelEnv,clock)

@cocotb.test()
@repot_test
async def gpio_all_i_pu(dut):
    caravelEnv,clock = await test_configure(dut,timeout_cycles=111154138)
