...
async def gpio_all_o(dut):
    caravelEnv = await test_configure(dut,timeout_cycles=1111538624)
    await gpio_all_o_seq(dut,caravelEnv)

@cocotb.test()
@repot_test
async def gpio_all_i(dut):
    caravelEnv = await test_configure(dut,timeout_cycles=111156837)
    await gpio_all_i_seq(dut,caravelEnv)

@cocotb.test()
@repot_test
async def gpio_all_i_pu(dut):
    caravelEnv = await test_configure(dut,timeout_cycles=111154138)
