...
class GetDefines:
        ...
        for line in define_lines:
            ...
            if match:
                if "`" in match[0][1]:
                    ...
                    val = self.verilog_num_parser(match[0][1]) if match[0][1] != '' else match[0][1]
    ...
    def get_include_paths_verilog(self, verilog_file):
        ...
        for include_match in include_matches:
            ...
            if include_path.endswith("defines.v") or include_path.endswith("user_defines.v"):
                define_paths.append(include_path)
        return define_paths

    def get_include_paths_command(self, verilog_file):
        with open(verilog_file, 'r') as file:
            ...
            for line in file:
                ...
                if line and not line.startswith('#'):
                    ...
                    line = line.replace('$(VERILOG_PATH)', verilog_path)        
                    # Extract file path from command
                    if line.startswith('-v'):
                        file_path = line.split(' ')[1]
                        if file_path.endswith("defines.v") or file_path.endswith("user_defines.v"):
                            define_paths.append(file_path)
        return define_paths
