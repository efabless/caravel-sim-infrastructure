...
class RunTest:
    ...
    def write_verilator_includes_file(self):
        self.verilator_dirs = ' '
        self.verilator_dirs += f'+incdir+{self.paths.USER_PROJECT_ROOT}/verilog/rtl/'
        self.test.set_user_project()
    # verilator function

    def runTest_verilator(self):
        if self.test.sim in ["GL", "GL_SDF"]:
            raise EnvironmentError("GL simulator not supported for verilator, use iverilog")
        self.write_verilator_includes_file()

        if not os.path.isfile(f"{self.test.compilation_dir}/sim.vvp") or self.args.compile:
            self.verilator_compile()
        self.verilator_run()

    def verilator_compile(self):
        macros = " -D" + " -D".join(self.test.macros)
        verilotor_debug = " --debug --gdbbt" if self.args.verbosity == "debug" else ""
        verilator_command = (
            f"cd {self.test.compilation_dir} && "
            f" verilator {macros} {self.verilator_dirs} {verilotor_debug} --binary -j 0 --timing -bbox-unsup -Wno-fatal "
        ...
        docker_compilation_command = self._iverilog_docker_command_str(verilator_command)
        self.run_command_write_to_file(
