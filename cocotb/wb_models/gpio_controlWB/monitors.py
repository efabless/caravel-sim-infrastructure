...
class DefaultMonitor(Monitor):
    ...
    async def _monitor_recv(self):
        ...
        for key2, signal in self.interfaces.items():
            signal["val"] = list(
                self.block_path._id(signal["signal"], False).value.binstr
            )
...
class SerialMonitor(Monitor):
    ...
    async def _monitor_recv(self):
        while True:
            ...
            for key2, signal in self.interfaces.items():
                signal["val"] = self.block_path._id(
                    signal["signal"], False
                ).value.binstr
            cocotb.log.debug(
                f"[{__class__.__name__}][{self.name}] interface = {self.interfaces}"
            )
            if self.is_output:
                await Timer(1, "ns")
            self._recv(self.interfaces)
    ...
    async def _monitor_recv(self):
        while True:
            ...
            signal = self.block_path._id(
                self.interfaces[f"{self.pad_name}"]["signal"], False
            )
            await Edge(signal)
            for key2, signal in self.interfaces.items():
                signal["val"] = self.block_path._id(
