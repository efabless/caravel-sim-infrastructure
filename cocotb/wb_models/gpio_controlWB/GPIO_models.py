...
class GPIO_models:
    ...
    def serial_model(self, trans):
        ...
        if reset == 0:
            ...
            if load == "1":
                ...
                cocotb.log.debug(
                    f"[GPIO_models][serial_model][{self.name}] loaded value = {self.gpio_configuration} string {''.join(self.gpio_configuration)} hex = {hex(int(''.join(self.gpio_configuration),2))}"
                )
                ...
                cocotb.log.debug(
                    f"[GPIO_models][serial_model][{self.name}] shifted value = {self.shift_array}"
                )
        if self.checkers:  # expect the serial output
            ...
            self.serial_out_if["data"][
                "val"
            ] = f"{self.shift_arr_deleted_val}"  # data is being moved in negative edge that's why taking the old deleted val in the postive edge
            if load == "1":
                self.serial_out_if["data"][
                    "val"
                ] = "0"  # data is being moved in negative edge that's why taking the old deleted val in the postive edge
    ...
    def default_model(self, trans):
        ...
        cocotb.log.debug(
            f"[GPIO_models][default_model][{self.name}] default value = {self.gpio_configuration}"
        )
        self.gpio_config_cov(
            ["0", "0", "0", "0"]
        )  # buffer value to initalize the coverages
        # sys.exit()
    ...
    def gpio_config_cov(
        ...
        @CoverPoint(
            f"top.caravel.gpio_controls.{self.name}.config",
        ...
        @CoverPoint(
            f"top.caravel.gpio_controls.{self.name}.data_transfer.mgmt_gpio",
        ...
        @CoverPoint(
            f"top.caravel.gpio_controls.{self.name}.data_transfer.gpio_mgmt",
        ...
        @CoverPoint(
            f"top.caravel.gpio_controls.{self.name}.data_transfer.user_gpio",
        ...
        @CoverPoint(
            f"top.caravel.gpio_controls.{self.name}.data_transfer.gpio_user",
