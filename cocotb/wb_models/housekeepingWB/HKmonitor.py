    ...
    async def _monitor_recv(self):
        ...
        while True:
            ...
            if "_valid_cycle" in self.interfaces:  # for interfaces with valid signal
                signal = self.block_path._id(
                    self.interfaces["_valid_cycle"]["signal"], False
                ).value.binstr
                if signal is not "1":
                    continue
            ...
            if self.name == "HKoutputsMonitorwishbone":
                if not self.interfaces["data"]["val"].is_resolvable:
                    self.interfaces["data"]["val"] = BinaryValue(
                        value=0, n_bits=self.interfaces["data"]["val"].n_bits
                    )
            cocotb.log.debug(
                f"[HKmonitor][_monitor_recv] interface at monitor {self.name} self.interfaces {self.interfaces}"
            )
            ...
            if "_hold" in self.interfaces:
                if old_trans_hold is None:
                    ...
                    if old_trans_hold != self.interfaces:
                        cocotb.log.error(
                            f"[HKmonitor][_monitor_recv] interface at monitor {self.name} change value before hold value is asserted \nold value {old_trans_hold} \nnew value {self.interfaces}"
                        )
                        raise TestFailure
